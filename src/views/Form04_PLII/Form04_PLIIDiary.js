...
const Form04_PLIIDiary = ({navigation}) => {
  ...
  const fetchdata = async () => {
    ...
    setRefreshing(true);
    const rawDiary = await getDiaryForm04_PLII();
    try {
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            await Storage.setItem('form04_PLII', JSON.stringify(newData)); // update lại data vừa xoá
    ...
    <View style={styles.boxbtn}>
      <TouchableOpacity
        onPress={async () => {
          ...
          if (netInfo.isConnected) {
            dataTemp = await getDetailForm04_PLII_Id(id);
          ...
          const result = await ExportPDF04_PLII(dataTemp);
          setIsPDFLoading(false);
          result
            ? navigation.navigate('ViewPDF',{data:dataTemp,nameParams:'Mẫu Biên bản kiểm tra tàu cá rời cảng'})
      ...
      <TouchableOpacity
        onPress={async () => {
          ...
          let tempData;
          if (netInfo.isConnected) {
            tempData = await getDetailForm04_PLII_Id(id);
          ...
          if (tempData) ExportPDF04_PLII(tempData);
      ...
      <TouchableOpacity
        onPress={async () => {
          ...
          if (tempData) PrintfPDF04_PLII(tempData);
...
export default Form04_PLIIDiary;
